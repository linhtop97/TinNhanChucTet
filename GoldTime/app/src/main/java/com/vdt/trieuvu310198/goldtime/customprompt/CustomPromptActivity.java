...
//                        toast("onColorSelected: 0x" + Integer.toHexString(selectedColor));
    ...
    private void showDialogTimepicker() {
        ...
        if (positionRV != -1) {
            timePickerCustom.setCurrentHour(Integer.parseInt(tvTimepicker.getText().toString().substring(0, 2)));
            timePickerCustom.setCurrentMinute(Integer.parseInt(tvTimepicker.getText().toString().substring(3, 5)));
        }
        dialogTimepicker.findViewById(R.id.bt_ok_dialog_timepicker_prompt).setOnClickListener(new View.OnClickListener() {
            @RequiresApi(api = Build.VERSION_CODES.M)
            @Override
            public void onClick(View view) {
                time = String.format("%02d", timePickerCustom.getCurrentHour()) + ":" + String.format("%02d", timePickerCustom.getCurrentMinute());
